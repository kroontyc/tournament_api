...
class Matchs extends Controller
{
  public function generateTournamentMatch(Request $request)
  {
      ...
      // Obtenha os participantes do torneio
      $participants = Participant::where('tournament_id', $tournamentId)->get();

      // Verifique se já existe um match para este torneio
      ...
      // Embaralhe os participantes para gerar as chaves aleatoriamente
      $participants = $participants->shuffle();

      // Filtre os participantes elegíveis
      ...
      // Contagem de participantes elegíveis
      $numParticipants = $eligibleParticipants->count();

      // Ajuste no cálculo do max_keys:
      if ($numParticipants >= 2) {
          $maxKeys = floor($numParticipants / 2); // O número de chaves é sempre metade do número de participantes
      } else {
          $maxKeys = 0; // Se não há participantes suficientes, não há chaves
      }

      // Lista para armazenar os matches gerados
      $matches = [];
      
      // Cria as chaves (matches) enquanto houver participantes elegíveis
      while ($eligibleParticipants->count() >= 2) {
          ...
          if ($secondParticipantKey === false) {
              // Se não achar um adversário de time diferente, busca qualquer participante com os mesmos critérios
              $secondParticipantKey = $eligibleParticipants->search(function ($participant) use ($firstParticipant) {
                  return $participant->categorie == $firstParticipant->categorie &&
                      $participant->sex == $firstParticipant->sex &&
                      $participant->weight == $firstParticipant->weight;
          ...
          // Se um segundo participante for encontrado, cria o match
          if ($secondParticipantKey !== false) {
              ...
              // Adiciona o valor de max_keys aos dados do match
              $matchData['max_keys'] = $maxKeys;
   ...
   public function getTournamentMatchesById(Request $request, $id)
  {
    ...
    $tournamentId = $id;
    $perPage = $request->query('perPage', 20000);
